...
class PlayerBeatListener implements AudioListener
{
  private BeatDetect beat;
  private AudioPlayer source;
    
  PlayerBeatListener(BeatDetect beat, AudioPlayer source)
  {
    this.source = source;
...
class InputBeatListener implements AudioListener
{
  private BeatDetect beat;
  private AudioInput mic;
  
  InputBeatListener(BeatDetect beat, AudioInput source)
  {
    this.mic = source;
    this.mic.addListener(this);
    this.beat = beat;
  }
...
void setup()
{
  ...
  //song = minim.loadFile("straight.mp3", 1024);
  //song.play();
  ...
  beat = new BeatDetect(mic.mix.size(), mic.sampleRate());
  ...
  bl = new InputBeatListener(beat, mic);  
